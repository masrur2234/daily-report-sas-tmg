...
function SortIcon({ sortKey, sortDir, col }: { sortKey: string; sortDir: string; col: string }) {
...
function KreditTable({ data, filters }: { data: KreditAO[]; filters: DataTablesProps['filters'] }) {
  ...
  const thClass = "py-2.5 px-3 text-xs font-semibold text-white text-center select-none whitespace-nowrap"
  const tdClass = "py-2 px-3 text-xs text-right font-mono tabular-nums whitespace-nowrap"
            ...
            <thead className="sticky top-0 z-10">
              <tr className="bg-blue-700">
                ...
                <th className={thRightClass} style={{ cursor: 'pointer' }} onClick={() => toggleSort('noa')}>
                  NOA <SortIcon sortKey={sortKey} sortDir={sortDir} col="noa" />
                </th>
                <th className={thRightClass} style={{ cursor: 'pointer' }} onClick={() => toggleSort('os')}>
                  OS (Baki Debet) <SortIcon sortKey={sortKey} sortDir={sortDir} col="os" />
                </th>
                <th className={thRightClass} style={{ cursor: 'pointer' }} onClick={() => toggleSort('lancar')}>
                  LANCAR <SortIcon sortKey={sortKey} sortDir={sortDir} col="lancar" />
                </th>
                <th className={thRightClass} style={{ cursor: 'pointer' }} onClick={() => toggleSort('dpk1to30')}>
                  01-30 <SortIcon sortKey={sortKey} sortDir={sortDir} col="dpk1to30" />
                </th>
                <th className={thRightClass} style={{ cursor: 'pointer' }} onClick={() => toggleSort('dpk')}>
                  DPK <SortIcon sortKey={sortKey} sortDir={sortDir} col="dpk" />
                </th>
                <th className={thRightClass} style={{ cursor: 'pointer' }} onClick={() => toggleSort('totNpl')}>
                  TOT NPL <SortIcon sortKey={sortKey} sortDir={sortDir} col="totNpl" />
                </th>
                <th className={thRightClass} style={{ cursor: 'pointer' }} onClick={() => toggleSort('rr')}>
                  RR (%) <SortIcon sortKey={sortKey} sortDir={sortDir} col="rr" />
                </th>
                <th className={thRightClass} style={{ cursor: 'pointer' }} onClick={() => toggleSort('npl')}>
                  NPL (%) <SortIcon sortKey={sortKey} sortDir={sortDir} col="npl" />
                </th>
              </tr>
            </thead>
            <tbody>
                ...
                <>
                  {filtered.map((item, idx) => (
                    <tr key={item.id} className={`border-b ${idx % 2 === 0 ? 'bg-white' : 'bg-blue-50/40'} hover:bg-blue-50/80 transition-colors`}>
                      ...
                      <td className="py-2 px-3 text-xs font-medium text-left whitespace-nowrap">{item.nama}</td>
                      ...
                      <td className={`${tdClass} text-teal-700`}>{formatRupiah(item.dpk1to30)}</td>
                  ...
                  <tr className="bg-green-600 text-white font-bold">
                    ...
                    <td className={tdClass}>{formatRupiah(totals.dpk1to30)}</td>
