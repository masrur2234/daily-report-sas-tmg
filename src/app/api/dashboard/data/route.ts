...
export async function GET(request: NextRequest) {
  try {
    ...
    const upload = await db.dashboardUpload.findFirst({
      where: { uploadDate },
      include: { kreditAO: true, mutasiAO: true, tabunganFO: true, depositoFO: true }
    });
    ...
    // RR = SUM(LANCAR) / SUM(OS) * 100
    const avgRR = totalOsKredit > 0 ? (totalLancar / totalOsKredit) * 100 : 0;
    const totalTotNpl = upload.kreditAO.reduce((sum, k) => sum + k.totNpl, 0);
    // NPL = SUM(TOTNPL) / SUM(OS) * 100
    const avgNPL = totalOsKredit > 0 ? (totalTotNpl / totalOsKredit) * 100 : 0;
